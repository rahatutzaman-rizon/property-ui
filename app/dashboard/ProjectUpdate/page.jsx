...
import { Edit2, Download, Search, Plus, X } from 'lucide-react';
...
const ProjectTable = () => {
  ...
  const fetchProjects = async () => {
    try {
      const response = await axios.get('https://asset-server.bdcare.vip/projects');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('https://asset-server.bdcare.vip/projects', formData, {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      await axios.put(`https://asset-server.bdcare.vip/projects/${updateProject._id}`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      fetchProjects();
  ...
  const handleDownload = async (imageUrl) => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', 'project_image.jpg');
          ...
          <tbody>
            {filteredProjects.length > 0 ? (
              filteredProjects.map((project, index) => (
                <tr key={project._id} className="border-t hover:bg-gray-50 transition">
                  <td className="p-4 text-gray-800">{index + 1}</td>
                  {/* <td className="p-4 text-gray-800 break-all">{project._id}</td> */}
                  <td className="p-4">
              ...
              <tr>
                <td className="p-4 text-center" colSpan="5"></td>
              </tr>
            )}
          </tbody>
        </table>
            ...
            <form onSubmit={handleUpdate} className="space-y-4">
              <div>
                ...
                >
                  <option value="ongoing">Ongoing</option>
                  <option value="completed">Completed</option>
                  <option value="on-hold">Upcomming</option>
                </select>
              </div>
              <div>
